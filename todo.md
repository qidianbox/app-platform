...
## 2026-01-11 BaaS功能开发 - 第二阶段完成

### 文档增删改查前端界面
- [x] 数据查看对话框完善
- [x] 新增数据功能（根据数据模型字段动态生成表单）
- [x] 编辑数据功能
- [x] 删除数据功能
- [x] 数据搜索功能
- [x] 数据分页功能
...
### 字段验证功能
- [x] 后端CreateDocument添加字段验证
- [x] 后端UpdateDocument添加字段验证
- [x] 必填字段验证
- [x] 字段类型验证（string/number/boolean/array/object）
...
## 2026-01-11 BaaS功能重构完成

### 配置中心 - 数据模型管理
- [x] BaaS数据服务分组添加到配置中心侧边栏
- [x] 数据模型管理页面（创建/编辑/删除数据模型）
- [x] 字段定义功能（字段名、类型、必填、默认值）
- [x] 权限配置功能（读取/创建/更新/删除权限）
- [x] API文档自动生成（包含认证信息、接口列表、字段定义）
- [x] API端点展示和复制功能
...
### 配置中心 - 功能生成器
- [ ] API生成器：定义数据模型后自动生成CRUD API
- [ ] 页面生成器：根据数据模型自动生成前端页面（列表页、表单页、详情页）
- [ ] 代码生成器：生成可下载的前端/后端代码文件
- [ ] 代码预览功能
- [ ] 代码下载功能（ZIP打包）
...
- [x] 代码生成器功能
  - [x] 选择数据模型
  - [x] 选择生成内容（后端API、前端页面、数据库迁移）
  - [x] 代码预览
  - [x] 一键复制代码
  - [x] 下载ZIP压缩包功能

### 测试验证
- [x] API生成器测试 - 成功生成Go API代码
- [x] 页面生成器测试 - 成功生成Vue页面代码和预览
- [x] 代码生成器测试 - 成功生成完整代码包


## 2026-01-11 功能生成器重构 - 生成的功能直接可用

### 核心概念
- 配置中心：定义数据模型、配置字段、生成功能
- 工作台：展示和使用生成的功能页面
- 生成的功能直接在系统中可用，不是下载代码

### 配置中心功能
- [ ] 数据模型管理（创建/编辑/删除）
- [ ] 字段定义（字段名、类型、必填、默认值）
- [ ] 功能生成按钮（点击后自动创建API和页面）
- [ ] 生成状态显示（已生成/未生成）

### 工作台功能
- [ ] 动态菜单：显示所有已生成功能的页面入口
- [ ] 动态页面：根据数据模型定义自动渲染CRUD页面
- [ ] 数据操作：新增、编辑、删除、搜索、导入导出

### 后端支持
- [ ] 动态API路由（根据数据模型自动注册CRUD路由）
- [ ] 数据模型状态字段（is_generated标记是否已生成功能）


## 2026-01-11 功能生成器修复

### 问题分析
- [x] 分析功能生成器调用失败原因（API路径错误）
- [x] 前端调用了 `/api/v1/baas/...` 但后端路由是 `/baas/...`

### 修复实施
- [x] 修复前端generateFeature函数的API调用路径
- [x] 将 `request({url: '/api/v1/baas/...'})` 改为 `request.post('/baas/...')`
...
### 功能生成器完整流程
1. ✅ 在配置中心创建数据模型（定义字段结构）
2. ✅ 点击"生成功能"按钮，状态变为"已生成"
3. ✅ 工作台自动出现对应的数据管理菜单
4. ✅ 可以进行数据的增删改查操作
...
- 菜单管理页面显示所有已生成的功能列表
- 每个功能有一个"显示/隐藏"开关
- 只有开关打开的功能才会显示在工作台侧边栏

### 后端开发
- [ ] 在collections表添加is_visible字段（控制是否显示在工作台）
- [ ] 添加更新菜单显示状态的API
- [ ] 修改获取collections的API返回is_visible字段

### 前端开发
- [ ] 在配置中心侧边栏添加"菜单管理"入口
- [ ] 实现菜单管理页面（显示已生成功能列表+开关）
- [ ] 修改工作台侧边栏，只显示is_visible=true的功能
...
### 需求
- 功能生成后，在配置中心增加"菜单管理"功能
- 用户可以控制哪些已生成的功能显示在工作台侧边栏

### 实现
- [x] 后端添加is_visible字段到data_collections表
- [x] 后端添加ToggleVisibility API (PUT /baas/apps/:appId/collections/:id/visibility)
- [x] 前端实现菜单管理页面（显示已生成功能列表+开关控制）
- [x] 修改工作台侧边栏根据is_visible字段过滤菜单
- [x] 测试完整流程（开关开启显示菜单，关闭隐藏菜单）

### 功能流程
1. 在「数据模型管理」中创建数据模型并点击「生成功能」
2. 生成后的功能会出现在「菜单管理」列表中
3. 开启「工作台显示」开关，该功能将显示在工作台侧边栏
4. 关闭开关后，功能仍然存在，只是不在工作台显示


## 2026-01-11 版本管理功能开发

### 需求
为生成的功能和开发的模块增加版本管理，支持版本追踪、回滚、发布等特性

### 功能版本管理（数据模型）
- [ ] 设计feature_versions数据表
- [ ] 后端实现版本CRUD API
- [ ] 后端实现版本发布/回滚API
- [ ] 前端实现版本列表页面
- [ ] 前端实现版本对比功能
...
### 模块版本管理（配置）
- [ ] 设计module_versions数据表
- [ ] 后端实现模块版本API
- [ ] 前端实现模块版本管理页面
- [ ] 支持多环境版本（dev/test/prod）
...
### 修复
- 修改API调用路径从 `/api/v1/baas/...` 改为 `/baas/...`
...
### 功能特性
- 版本列表显示（版本号、状态、变更说明、创建人、创建时间、发布时间）
- 创建新版本功能
- 查看版本详情
- 发布版本
- 版本对比
...
### 功能版本管理（数据模型）
- [x] 后端实现版本管理API（创建、列表、发布、回滚、对比）
- [x] 前端实现版本管理页面（FeatureVersions.vue）
- [x] 从数据模型管理页面可以点击"版本管理"按钮进入
- [x] 支持版本状态（草稿/已发布/已废弃）
- [x] 支持版本对比功能

### 模块版本管理（用户管理、消息推送等）
- [x] 后端实现模块版本管理API（创建、列表、发布、回滚、对比）
- [x] 前端实现模块版本管理页面（ModuleVersions.vue）
- [x] 从模块配置页面可以点击"版本管理"按钮进入
- [x] 支持多环境（开发/测试/生产）
- [x] 支持版本状态（草稿/已发布/已废弃）
- [x] 支持版本对比功能
